...
def create_inputs(inputs_list):
	html_inputs = ''
	# take each name and create an <INPUT> tag
...
def do_form(name, the_inputs, method="POST", text="Submit"):
	with open('templates/form.html') as formf:		# grab the template from your disk
		form_text = formf.read()
	inputs = create_inputs(the_inputs)				# create the list of <INPUT> tags
	form = Template(form_text)						# create a template form
